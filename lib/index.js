...
function Connection() {
    var self = this,
        ...
        mode = {},
        nick = null,
        options = {           // Default connection options
            autoBounce: true,
            host: null,
    ...
    function onconnection() {
        ...
        if (options.secure && !socket.authorized) {
        ...
        reader.on('line', function(line) {
            ...
            // Translate the reply code (e.g. '005') in the reply string (e.g. 'RPL_WELCOME') as in replies.js
        ...
        // 1) 'simple': classic authentication process. A PASS command is sent to the server before the NICK and USER
        //    commands.
        // 2) 'nickserv': authentication via nickserv. After registration send an identify command to nickserv.
    ...
    //  1) cap        2) info       3) invite     4) help       5) ison       6) join       7) kick       8) kill
    //  9) list      10) mode      11) motd      12) names     13) notice    14) nick      15) oper      16) part
    // 17) pass      18) ping      19) pong      20) privmsg   21) quit      22) time      23) topic     24) trace
    // 25) user      26) userhost  27) users     28) version   29) wallops   30) who       31) whois     32) whowas
    //
    // Every method accepts as many arguments as specified by the corresponding command in the RFC. For example:
    ...
    //
    // Parameters are merged together and a colon to the last parameter is automatically added if necessary.

    commands.forEach(function(command) {
        // The commands variable (from require('./commands');) contains the list of all IRC commands. For every command
        // we add a function.
    ...
    //
    // If you pass at least one parameter to these methods, the corresponding command is sent to the IRC server.
    ...
    self.start = function(opts, callback) {
        ...
        // options.caps is key-value store that maps capabilities to callbacks.
        //
        // var options = {
        //     caps: {
        //          "multi-prefix": function(next) { ... },
        //          "sasl": function(next) { ... },
        ...
        // };
        // 
        // The client can also request capabilities in form of an array of strings. In this case the array is
        // transformed into a map with null callbacks.
        ...
        socket.on('close', function() {
            socket = host = nick = user = real = null;
            mode = {};
            state = 'closed';
            self.emit('close');
        });
    ...
    self.on('cap', function(from, x, sub, caps) {
        ...
        // Handle the message only if it is received before registration.
        if (state === 'connected') {
            switch (sub) {
            case 'ACK':
                callbacks = [];
                ...
                // options.caps is key-value store that maps capabilities to callbacks. The client can request
                // capabilities in the options of the connection.
                //
                // var options = {
                //     caps: {
                //          "multi-prefix": function(next) { ... },
                ...
                //     ...
                // };
                // var connection = new irc.Connection(options);
                // 
                // When a capability is set for this connection, the corresponding callback is called in a lockstep
                // fashion. To let the next callback to be called the user must explicitly invoke the function (next(),
                // in the above example) passed as argument to the callback. When no callback is left a CAP END command
                ...
                break;
            case 'LS':
                // It is possible that some capabilities are not supported by the server; the implementation requests
